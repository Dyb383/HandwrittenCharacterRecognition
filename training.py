...
def load_data(mat_file_path, width=28, height=28, max_=None, debug=True):
    ...
    if debug == True: _len = len(training_images)
    for i in range(len(training_images)):
        if debug == True: print('%d/%d (%.2lf%%)' % (i + 1, _len, ((i + 1)/_len) * 100), end='\r')
        training_images[i] = rotate(training_images[i])
    if debug == True: print('')

    # Reshape testing data to be valid
    if debug == True: _len = len(testing_images)
...
def build_net(training_data, width=28, height=28, debug=False):
    ...
    pool_size = (4,4)  # size of pooling area for max pooling
...
#drops some number of inputs and flattens data
...
#adadelta optimizer adapts learning rates based on a moving window of gradient
...
#This way, Adadelta continues learning even when many updates have been done
    ...
    if debug == True: print(model.summary())
    return model
...
def train(model, training_data, callback=True, batch_size=256, epochs=5):
    ...
    score = model.evaluate(x_test, y_test, debug=0)
...
if __name__ == '__main__':
    ...
    training_data = load_data(args.file, width=28, height=28, max_=None, debug=False)
    model = build_net(training_data, width=28, height=28, debug=False)
    train(model, training_data, epochs=32)
